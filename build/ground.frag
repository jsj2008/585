...
void main() {
	...
	vec4 texValue;
	vec4 groundValue = texture2D(groundTex, gl_TexCoord[0].st);
	vec4 cliffValue = texture2D(cliffTex, gl_TexCoord[0].st);
	float uppity = dot(-trueNormal, vec3(0,1,0));
	
	if (uppity < 0.9) texValue = cliffValue;
	else if (uppity < 0.91) texValue = ((1.0-(uppity-0.9)*100.0) * cliffValue) + (uppity-0.9)*100.0*groundValue;
	else texValue = groundValue;
	
	vec3 bumpG = normalize(texture2D(groundNormalMap, gl_TexCoord[0].st).xyz * 2.0 - 1.0);
	vec3 bumpC = normalize(texture2D(cliffNormalMap, gl_TexCoord[0].st).xyz * 2.0 - 1.0);
	vec3 bump;	
	if (uppity < 0.9) bump = bumpC;
	else if (uppity < 0.91) bump = ((1.0-(uppity-0.9)*100.0) * bumpC) + (uppity-0.9)*100.0*bumpG;
	else bump = bumpG;
